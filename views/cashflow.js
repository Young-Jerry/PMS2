...
const CashflowView = (() => {
    ...
    container.innerHTML = `
      <div class="view-enter">
        ...
        <div class="pms-card section-gap">
          ...
          <div class="pms-card-body">
            <div class="pms-form-grid" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px;">
              <div class="pms-field">
  ...
  function runCalc(container) {
    ...
    result.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(190px,1fr));gap:12px;padding:16px;background:var(--bg-elevated);border-radius:var(--r-md);">
        <div><div class="stat-label">Total Amount</div><div style="font-family:var(--font-mono);font-size:15px;">${PmsUI.currency(tx.totalAmount)}</div></div>
        ${!wacc || side === 'sell' ? `
          <div><div class="stat-label">Broker Commission</div><div style="font-family:var(--font-mono);font-size:15px;">${PmsUI.currency(tx.commission)}</div></div>
          <div><div class="stat-label">SEBON Fee</div><div style="font-family:var(--font-mono);font-size:15px;">${PmsUI.currency(tx.sebonFee)}</div></div>
          <div><div class="stat-label">DP Charge</div><div style="font-family:var(--font-mono);font-size:15px;">Rs ${tx.dpCharge}</div></div>
        ` : ''}
        <div><div class="stat-label">${side === 'sell' ? 'You Receive' : 'Total Payable'}</div><div style="font-family:var(--font-mono);font-size:16px;font-weight:700;">${PmsUI.currency(tx.totalPayable)}</div></div>
        <div><div class="stat-label">Cost Per Share</div><div style="font-family:var(--font-mono);font-size:15px;">${PmsUI.currency(tx.costPerShare)}</div></div>
      </div>
    `;
  }
